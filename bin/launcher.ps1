...
Function Main-Menu {
    ...
    Write-Host "7) " -NoNewline; Write-Host "VIEW LOGS " -ForegroundColor Red
    Write-Host "8) " -NoNewline; Write-Host "DEBUG MODE " -ForegroundColor Red -NoNewline; Write-Host "(LSL Test)"
    Write-Host "q) EXIT"
    Write-Host "--------------------------------------------------" -ForegroundColor Magenta
    ...
    Switch ($choice) {
        ...
        "7" { Get-Content -Tail 50 /root/.bridge.log }
        "8" { Start-Process python "bci_interface/sender.py"; Start-Sleep -s 2; python -c "from pylsl import resolve_stream; print('LSL Streams:', resolve_stream())"; Stop-Process -Name python -Force }
        "q" { Exit }
